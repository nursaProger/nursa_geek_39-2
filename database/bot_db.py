...
class Database:
    ...
    def sql_check_banned_user(self, tg_id):
        potential = self.sql_select_ban_user(tg_id)
        if potential:
    ...
    def insert_profile(self, tg_id, nickname, bio, age, married, gender, hobby, height, photo):
        self.cursor.execute(
            sql_queries.INSERT_PROFILE_QUERY,
            (None, tg_id, nickname, bio, age, married, gender, hobby, height, photo)
        )
        self.connection.commit()
